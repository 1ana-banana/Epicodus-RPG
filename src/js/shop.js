...
export default class Shop {
  ...
  SellItem(character, item) {
    ...
    if (invItem) {
      character.money += Math.floor(invItem.value * .8);
      return true;
    } else {
      return false;
    }
  ...
  BuyItem(character, item) {
    ...
    if (character.inventory.IsFull() || character.money < Math.floor(value)) {
      return false;
    } else {
      let boughtItem = this.inventory.RemoveItem(item);
      character.inventory.AddItem(boughtItem);
      character.money -= Math.floor(boughtItem.value * 1.5);
      return true;
    }
  }
}